...
class ClouDNS_SDK
{
	...
	{
		$CI =& get_instance();
		$params = $CI->config->item('cloud_dns');
		$is_subuser = $params['is_subuser'];

		if ($is_subuser == false) {
			$this->user_type = 'auth-id';
		} else {
			if (is_numeric($params['auth-id'])) {
				$this->user_type = 'sub-auth-id';
			} else {
				$this->user_type = 'sub-auth-user';
			}
		}
		$this->id = $params['auth-id'];
		$this->password = $params['auth-password'];
	}

	public function setAuthInfo($auth_id, $auth_password, $is_subuser = false)
	{
		if ($is_subuser == false) {
			$this->user_type = 'auth-id';
		} else {
